<template>
	<view
		class="u-switch"
		:class="[(valueCom == true||valueCom == 1) ? 'u-switch--on' : '', disabled ? 'u-switch--disabled' : '']"
		@tap="onClick"
		:style="[switchStyle]"
	>
...
export default {
	...
	computed: {
		...
		switchStyle() {
			...
			style.backgroundColor =  (this.valueCom==true || this.valueCom==1) ? this.activeColor : this.inactiveColor;
			return style;
		},
		loadingColor() {
			return (this.valueCom==true || this.valueCom==1) ? this.activeColor : null;
		}
	},
	methods: {
		onClick() {
			if (!this.disabled && !this.loading) {
				...
				if(typeof this.valueCom=='boolean'){
